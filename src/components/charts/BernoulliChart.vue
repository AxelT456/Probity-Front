...
const chartDataForRender = computed(() => {
	...
	return {
		labels: pmfData.x,
		datasets: [{ type: 'bar', label: pmfData.label, data: pmfData.y, backgroundColor: 'lightgreen', borderColor: 'green' }]
	}
})
...
const chartOptions = computed(() => ({
	...
	plugins: { legend: { position: 'top' }, title: { display: true, text: props.chartData.title } },
	scales: {
